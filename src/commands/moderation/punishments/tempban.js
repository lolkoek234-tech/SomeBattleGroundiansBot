...
export default {
  ...
  async execute(interaction) {
    ...
    if (!duration) return interaction.editReply('❌ Invalid duration. Use format like 1h, 2d, 30m.');
    ...
    try {
      ...
      await interaction.editReply(`✅ Temp-banned ${user.tag} for ${durationStr} | Case #${record.id}`);
    } catch (err) {
